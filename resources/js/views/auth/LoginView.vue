  ...
  <div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8 bg-white p-8 rounded-lg shadow-md">
      <div class="text-center">
        <h1 class="text-2xl font-bold tracking-tight" :style="{ color: 'var(--brand-color)' }">{{ settingsStore.site_title }}</h1>
        <p v-if="settingsStore.site_tagline" class="text-sm text-gray-500 mt-1">{{ settingsStore.site_tagline }}</p>
...
const handleLogin = async () => {
    ...
    try {
        ...
        error.value = err.response?.data?.message || err.response?.data?.errors?.[0] || 'Invalid credentials';
    } finally {
        loading.value = false;
    }
};
